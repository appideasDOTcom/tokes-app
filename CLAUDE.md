...
- **Rendering the UI above 1x is not a scale argument.** `scripts/screenshots.sh
  ...
  - **The popovers are rendered twice, and that is the settled answer** — not a
    bug someone forgot to fix. `ImageRenderer` draws every
    `Image(systemName:)` inside a `.buttonStyle(.borderless)` button as the
    yellow placeholder plate, so the crisp 9x/6x `popover-*.png` always carry
    three of them in the header; the hosting path draws them correctly but
    rasterises at the layer's backing scale, which is 1.0 on every display here.
    So `--states` also writes `popover-*-hosted-*.png` at a true 1x with the
    header correct. **The operator ruled the app does not change to suit the
    capture tool** (`.borderless` stays) — crop the header off the crisp pair,
    or place the hosted pair. Both are regenerated by
    `scripts/screenshots.sh --states`.
...
- **Read `docs/CREDENTIALS.md` before touching any credential path.** It is
  ...
  **`README.md` carries a summary of the same mechanism and has drifted from it
  once already** — it described the pre-build-6 story (Claude Code keychain as
  the default, Copilot via `copilot_internal/user`) for long enough that it was
  the wrong thing to hand another agent. It now names `docs/CREDENTIALS.md` as
  the authority; when a credential path changes, update both in the same pass.
- **External copy claims come from `docs/marketing-facts.md` and the
  `product.facts` contract — never from this README or from memory.** The doc is
  the verified feature picture (one source reference per claim, re-derived at
  1.4.2 / build 6); the contract carries the facts a consumer hardcodes — name,
  URLs, requirements, licence, the network surface, the naming rules ("Claude
  *account plan* limits", always "GitHub Copilot"), and an
  `availability_status` block that says which install paths do not work yet.
  Anything user-facing about *what Tokes is* — a landing page, listing copy, a
  release note — starts there, and a claim that outlives its line in that
  contract is how a page ends up advertising an install that 404s.
